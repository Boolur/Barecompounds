-- Quarantine provider-ambiguous deliveries before Resend's 24-hour
-- idempotency window expires instead of risking a duplicate email.

alter table public.notification_outbox
  add column if not exists first_attempt_at timestamptz;

update public.notification_outbox
set first_attempt_at = coalesce(last_attempt_at, created_at)
